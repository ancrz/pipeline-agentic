...
With `memory: user`, Claude Code provides persistent agent memory at `~/.claude/agent-memory/Dokimos/`. Its contents persist across conversations.
...
```
Grep with pattern="<search term>" path="~/.claude/agent-memory/Dokimos/" glob="*.md"
```
...
With `memory: user`, Claude Code provides persistent memory at `~/.claude/agent-memory/Dokimos/`. Use that directory for durable learnings; the runtime manages its availability.
